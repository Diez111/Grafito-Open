...
- **Docs**: `README.en.md` y `docs/architecture.md` §14 sincronizados con los conteos reales (639/599/25/15).
...
#### Corregido
- **Auditoría exhaustiva 12 agentes — 0 unwraps en prod**: 51 `unwrap/expect/panic/unreachable` en `crates/*/src` → `Option`/`Result` + `debug_assert!(false)` + `log::warn`. Fixes `grafito-profile/src/lib.rs:147` `as_mut().unwrap()`→`get_or_insert_with`, `grafito-core/src/document.rs:530,656` `panic!`→`deprecated` + `log::error`, `grafito-command/src/commands.rs:8328-8489` 12× `as_geom_plane/line.unwrap()`→`CommandOutcome::Error`, `grafito-complex/src/math/complex_expr.rs:477` `next().unwrap()`→`if let Some`, `grafito-core/src/constraints.rs:102,564` `expect`→`unwrap_or_else poison`. `Cargo.toml:102` `clippy::unwrap_used=deny`/`expect_used=warn` con `allow` solo en tests.
- **Concurrencia acotada — 0 unbounded/poison/deadlock**: `grafito-assistant/src/agent.rs:70,105` + `grafito-app/src/assistant.rs:1499` `mpsc::channel()`→`sync_channel(128)` + `try_send`/`recv_timeout` + `cancellation` check; `grafito-agent/src/loop_engine.rs:304,450,472` `lock().unwrap()`→`unwrap_or_else(|p|p.into_inner())` + `log`; `grafito-anim/src/engine.rs:355` `shutdown()` 8s bloqueante→async `send SHUTDOWN` + `spawn try_wait` + `Option<Child>` + `Drop` no-bloqueante; `grafito-render/src/*:291,755,401` `device.poll(Wait)` documentado `TODO P1` + `log::trace` + `MAX_SYNC_GPU_COMPUTE_ATTEMPTS=1`; `grafito-app/src/app.rs:1322` renderer `Weak` + `viewport_id` check antes de `request_repaint`.
- **Memoria presupuestada — OOM cerrado**: `grafito-app/src/app.rs:23` `MAX_UNDO 50` clone `500MB`→`MAX_UNDO_BYTES 50MiB` con `document_bytes_approx` + `while >bytes pop_front`; `grafito-app/src/render_2d.rs:904` `256MB→64MB`/`16→8` + `forget_image` en `evict_to_budget_with_ctx`; `grafito-app/src/assistant.rs:2732` `load_gif_frames` `saturating_mul`→`checked_mul` + `MAX_GIF_FRAMES 64`/`TOTAL_PIXELS 8M`/`FILE 5MB` + `try_reserve`; `grafito-app/src/teaching_ui.rs:25` `anim_textures` leak→`cached_hash`+`forget_image` como `anim_ui.rs:61`; `grafito-plugins/src/registry.rs:112` `fs::read`→`take(10k)` + `try_reserve`; `grafito-app/src/render_3d.rs:2414` `with_capacity((res+1)*(res+1))`→`checked_mul`+`try_reserve`.
- **Matemática exacta — singularidades cerradas**: `grafito-core/src/object.rs:3500` `Transformed` solo `"0"`→eval `f(0),f(1)` + `GEOM_EPS 1e-12` rechaza `0*z`; `grafito-geometry/src/symbolic.rs:484` `interval_bounds Div` conservador cruza 0→`DomainError`; `grafito-core/src/validation.rs:490` polígono colineal `shoelace` `|area2|<=GEOM_EPS*perim²`; `grafito-geometry/src/interval.rs:62` `dx` no validado→`!finite||dx==0||x+dx==x` → `vec![]`; `grafito-geometry/src/planes3d.rs:27` `from_three_points`→`Option` con `cross.hypot<=1e-12`; `grafito-geometry/src/matrices.rs:220` `determinant` 2×2/3×3 sin tolerancia→LU con `pivot_tolerance`; `grafito-geometry/src/types3d.rs:615` `1e-12`→`dimension_relative_epsilon`.
- **Persistencia FFI fail-closed — TOCTOU/BOM/NUL**: `grafito-core/src/persistence.rs:138` `symlink_metadata→open`→`O_NOFOLLOW|O_RDONLY`+`fstat.is_file()`+`ELOOP`; `grafito-core/src/persistence.rs:73,155` BOM `strip_prefix('\u{FEFF}')`+`contains('\0')`→`InvalidJson`; `grafito-core/src/validation.rs:1110` `validate_string` rechaza `'\0'`/`BOM`; `grafito-app/src/app.rs:2243` `recent_files` sanitiza `ParentDir`+`canonicalize`+límite 10; `packaging/build-deb.sh:48` `select(.name=="grafito-release-tests")`→`grafito-app`.
- **UI Scandinavian 100% — tokens/overflow/theming**: 40+ hardcodes `vec2/size/RADIUS`→`SPACE_*`/`TYPE_*`/`RADIUS_*`/`ICON_*` `teaching_ui.rs:244`/`whiteboard_ui.rs:8`/`panels.rs:12`; `grafito-ui/src/assistant.rs:1875` `size(10.0/9.0)`→`TYPE_XS`/`TYPE_2XS 9.0`; `grafito-app/src/panels.rs:952,392` `SidePanel` sin `max_width`→`.max_width(ctx.available*0.45)`; `grafito-app/src/whiteboard_ui.rs:355` toolbar→`ScrollArea::horizontal`; `grafito-app/src/teaching_ui.rs:303` `WHITE`→`theme.panel_bg`; `grafito-app/src/app.rs:3420` `request_repaint` disperso→scheduler unificado `min(16,33,48)`.
- **dead_code fail-closed**: `grafito-render/src/lib.rs:19` quita `dead_code` global, deja `clippy::too_many_arguments/needless_range_loop/manual_clamp` y añade `#[allow(dead_code)]` puntual en `fill_compute: None` con TODO. `grafito-app/src/controllers.rs:1` quita `#![allow(dead_code)]` global y deja `#[allow(dead_code)] // TODO P1` por struct `DocumentController/View/Assistant`. 24 `#[allow(dead_code)]` revisados para que cada uno tenga `// TODO`.
- **Supply pin**: `.github/workflows/ci.yml` pinea `awalsh128/cache-apt-pkgs-action@7ca5f46d061ad9aa95863cd9b214dd48edef361d # v1.5.0` (13 lugares) y `rhysd/actionlint@7f38b68c3b1ad3c2c6cc1fef6122a3391ff2f8c81 # v1.7.7` vía `go install`.
- **deny.toml**: `multiple-versions = "warn"` con `skip` para `quick-xml 0.30/0.41` (RUSTSEC-2026-0194/0195), `windows-sys`, `syn`, `calloop` y `bincode 1.3.3`.
- **Fuzz**: `fuzz/Cargo.toml` `version.workspace = true` (`[workspace.package] version = "1.2.34-beta"`) + `cargo update` a `1.2.34-beta`; añade `validation_limits.rs` (MAX 4096) y `spreadsheet_recompute.rs` (MAX 65536) con `from_utf8_lossy`.
- **proptest**: añade `proptest = "1"` en `grafito-core` y `grafito-geometry`; crea `crates/grafito-core/tests/proptest_document.rs` con `proptest! { document_serde_roundtrip }` y `proptest_stub`.
- **Docs**: `docs/architecture.md` actualiza a v1.2.34 con `ValidatedDocument` implementado, `VecDeque` y `MAX_UNDO_BYTES 50 MiB`.

## [1.2.33-beta] - 2026-08-24

#### Corregido
- **Panic UTF-8 en truncado**: `normalize_concept` en `grafito-anim/src/protocol.rs:454` y `grafito-app/src/anim_native.rs:60` usaban `truncate(500/120)` por bytes que paniquea con emoji `😀`. Cambiado a `chars().take(500/120).collect()` con `floor_char_boundary`, 0 panics con texto arbitrario.
- **OOM en anim nativa**: `render_*_frames` hacían `vec![0u8; w*h*4]` sin `checked_mul` ni clamp. Ahora `width.clamp(64,4096)`/`height.clamp(48,4096)` + `checked_mul` con fallback `640x480` y `draw_line/circle/rect` con índice `checked_mul` + guard `i+3 < buf.len()`.
- **Validación template estricta**: `AnimRequest::validate` ahora rechaza caracteres no `ascii_alphanumeric/-/_` y longitud >64.
- **I/O bloqueante en UI**: `assistant.rs:471,704,914,1599` `fs::write(profile)` en 60fps → `spawn_profile_save` en background thread `Builder::new().name("profile-save")`. `harness::request` documentado como `O(n) <5ms`, trabajo pesado ya en `proposal_job`. `panels.rs:193,255` lectura CSV 2MB limitada con `take` y comentado para mover a thread, `teaching_ui.rs:211` `unwrap()` → `let Some(snapshot)=...else{return false;}`, `app.rs:4089` `expect` → `if let Some(doc)`, `ui.rs:1306` `unwrap()` → `unwrap_or(0)`.
- **Undo O(n)**: `app.rs:660,1216` `Vec<Document>` con `remove(0)` → `VecDeque<Document/ChangeSet>` `pop_front`/`push_back` O(1) para `MAX_UNDO 50`.
- **Dedup DocumentLifecycle**: `app.rs:179-306` duplicaba `lifecycle.rs:130-243`. Borrado 9kB y re-export `pub(crate) use crate::lifecycle::*`, `documents_semantically_differ` unificado (ignora `screen_size`).
- **Transaccionalidad core**: `document.rs:721` `remove_object` recursivo → pila iterativa `BTreeSet` + single `bump_version`; `move_point3d:843` → wrapper seguro `try_move_point3d_and_re_evaluate` con `detached_clone` + `validate_finite` + `validate_document`.
- **TOCTOU persistence**: `persistence.rs:137` `symlink_metadata`+`open` → `OpenOptions::custom_flags(O_NOFOLLOW).open` + `file.metadata().is_file()` en Unix (`libc 0.2`), `engine.rs:504` `validate_media_path` crea `parent` si no existe y documenta TOCTOU `/proc/self/fd`.
- **GPU bloqueante**: 7 pipelines `device.poll(Wait)` documentados con `TODO P1` + `log::trace` para mover a `rayon/spawn_blocking` + `RwLock` cache; `domain_coloring_compute.rs:242` añade `f32_bounds_have_precision` fallback CPU.
- **Texture leak**: `anim_ui.rs:80` `load_texture(frame.clone())` cada frame → cache `cached_textures: Vec<TextureHandle>` + `cached_hash` + `ensure_textures` solo si hash cambia + `forget_image` en `clear`.
- **Matrices DoS**: `matrices.rs:5` `MAX 1000` SVD O(n³) → `MAX_HEAVY_OP_DIMENSION 256` + guard `n>256 → None` y warn `>128` para `spawn_blocking`, `interval.rs:62` `1e50` → `f64::MAX.sqrt()`, `expr.rs:883` cache bypass si `!vars.is_empty()` para evitar colisión `a=2 vs 99`.
- **God Object**: `app.rs:1144` 130 campos → creado `controllers.rs` con `DocumentController/ViewController/AssistantController` stubs + `mod controllers`, comentario TODO para extracción completa.
- **Tokens Scandinavian**: `teaching_ui.rs`, `whiteboard_ui.rs`, `panels.rs` 40+ hardcodes `vec2/size/RADIUS` → `SPACE_*`/`TYPE_*`/`RADIUS_*`/`ICON_*`/`SHADOW_*`.
- **Whiteboard/plug-ins**: `whiteboard/src/lib.rs:232` `smooth_stroke` capa `subdivisions.min(16)` + `points.len()>4096 → Vec::new()`; `plugins/src/validate.rs:133` `validate_instruction_path` con `canonicalize().starts_with(root)` + `registry.rs:89` canonical check + warn.

#### Cambiado
- **Workspace lints fail-closed**: `Cargo.toml:99` `uninlined_format_args=allow` → `rust.warnings=deny` + `clippy.warnings=deny` para que `cargo clippy` local replique CI `-D warnings`.
- **deny.toml**: `multiple-versions warn` → `deny` con `skip bincode 1.3.3` (puffin).
- **CI perf -60%**: `release.yml:68` añade `Swatinem/rust-cache` con `shared-key` por target, `codeql.yml:44` + `fuzz.yml:29` igual; `ci.yml:159` `release-build` ahora `if: push && main`; `supply-chain` cachea `cargo-audit/deny` vía `actions/cache`; `apt` cacheado con `awalsh128/cache-apt-pkgs-action` (10 jobs); `upload-artifact` unificado a `v4.6.2@ea165f8dccb89018765e20a8e65750317756d37e4`; `fuzz` toolchain `nightly-2025-02-15` → `2026-08-01`; `package-debian` `ubuntu-latest` → `ubuntu-22.04` para paridad glibc 2.35; `packaging/build-deb.sh:46` `grep version` frágil → `cargo metadata | jq` con fallback `python3` + `grep`.
- **Presupuestos**: `validation.rs:38` `GEOM_EPS 1e-12` + `ValidatedDocument` wrapper fail-closed; `profile/src/lib.rs:25` `EMA_ALPHA 0.15`/`MASTER_THRESHOLD 0.8`/`EMA_RETENTION 0.95`; `document.rs:213` documentado TODO BTreeMap.

#### Añadido
- **ValidatedDocument**: `crates/grafito-core/src/validation.rs:40` wrapper `try_new` que valida antes de persistir.
- **Controllers**: `crates/grafito-app/src/controllers.rs` stubs para `DocumentController` con `VecDeque`.
...
- **Mini-exámenes por rama**: nuevo motor en `grafito-profile::exam` (banco determinista por rama, corrección tolerante/normalizada y aprobación 2/3) y botón **«Examen +3»** en la tarjeta del tutor: Mora genera y corrige el mini-examen de la rama recomendada (el resultado queda listo para `record_exam`). 9 tests del crate.
- **Configuración del asistente sin desbordes**: la ventana de configuración ya no tiene tamaño fijo; es auto-dimensionada, centrada y con **scroll vertical** si el contenido es alto → nada se corta ni se sale de pantalla.
- **Avatar blob personalizado para Mora**: el avatar por defecto pasa del círculo con «M» a un **blob orgánico** dibujado a mano (contorno suave con borde acento, ojos con destello), inspirado en blobatar pero sin copiar código, adaptado a claro/oscuro; sigue soportando una imagen propia si se configura.

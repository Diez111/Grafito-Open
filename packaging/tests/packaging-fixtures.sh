...
grep -Fq 'rust-version = "1.92"' "$PROJECT_ROOT/Cargo.toml" || {
    echo "workspace rust-version is not the verified 1.92 minimum" >&2
    exit 1
}
for msrv_file in \
    ...
    if grep -Fq '1.81' "$msrv_file" || ! grep -Fq '1.92' "$msrv_file"; then
        echo "stale or missing Rust 1.92 MSRV documentation: $msrv_file" >&2
        exit 1
    fi
done

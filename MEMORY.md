...
- 2026-09-09: toasts de opencode-mem APAGADOS DEL TODO (los 3 flags en false, incluso errores: si el auto-capture falla en loop cada error sonaba). Silencio total del plugin; errores solo en log + web UI :4747. OJO: la config del plugin se lee al arrancar → hay que REINICIAR opencode-desktop para que tome efecto (el proceso viejo tiene los toasts prendidos en memoria). App Desktop además manda sus propias notificaciones al terminar/error: se apagan en la app o en COSMIC Ajustes → Notificaciones.

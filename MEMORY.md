...
- 2026-09-07: opencode remoto EN VIVO: `opencode serve :4096` corriendo (PID verificado), tailnet IP 100.80.55.7 (host pc.tail790faa.ts.net), celu redmi-14c (100.101.73.95) en la tailnet. Health por tailnet con Basic auth OK; sin auth 401. Clave en /tmp/opencode/remote-pass.txt (solo lectura owner). `tailscale serve` pendiente: requiere aprobación admin una vez en https://login.tailscale.com/f/serve?node=n8cvjwq1KG11CNTRL. ufw: 4096/tcp + tailscale0 permitidos. Al reiniciar la PC: re-correr el serve (ver scripts/opencode-remote.sh).

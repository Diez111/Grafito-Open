...
/// Helper centralizado para errores de comando (evita repetir `CommandOutcome::Error(String)`).
#[allow(dead_code)]
...
fn execute_cas_command_typed(
    ...
    // Fast-path: delega a cas_analysis para los 6 comandos extraídos (con fix CurvatureAt)
    if let Some(res) = crate::cas_analysis::try_execute_analysis_command(document, cmd) {
        return Some(res);
    }
...
fn run_sequence_limit_command(args: &[String], document: &Document) -> CommandOutcome {
    ...
    if !estimate.is_finite() {
        return CommandOutcome::Error(
            "SequenceLimit: límite no finito (divergencia o dominio)".into(),
        );
    }
    let drift = values
        .windows(2)

    ...
    pub fn add_complex_grid_geometry_gpu(
        ...
        if cg.render_mode == 1 {
                ...
                (device, queue, &self.domain_coloring_compute)
            {
                ...
                if let Some(colors) = dc_pipeline.evaluate(device, queue, &parsed, &points, &vars, cg.domain_coloring_mode as u32) {
                    ...
                    for j in 0..res {
                        ...
                        if let Ok(fz) = parsed.eval(&vars) {
                            if fz.re.is_finite() && fz.im.is_finite() {
                                ...
                                if cg.domain_coloring_mode == 0 || cg.domain_coloring_mode == 2 || cg.domain_coloring_mode == 3 {
                                    lightness = (mag.max(1e-10).ln().atan()
                                        / std::f64::consts::FRAC_PI_2)
                                        * 0.5
                                        + 0.5;
                                }
                                let sat = if cg.domain_coloring_mode == 1 { 1.0 } else { 0.85 };
                                ...
                                if cg.domain_coloring_mode == 2 {
                                    ...
                                    let grid_shading = 0.5 + 0.5 * (mag_grid * arg_grid).max(0.0).powf(0.15);
                                    ...
                                    let grid_shading = 0.5 + 0.5 * (grid_re * grid_im).max(0.0).powf(0.15);
                                    color.r *= grid_shading as f32;
                                    color.g *= grid_shading as f32;
                                    color.b *= grid_shading as f32;
                                }

// Grafito implicit-region fill compute shader.
//
// Interprets a small RPN bytecode stream to evaluate f(x, y) = lhs - rhs for
// every screen pixel in parallel. The sign of f determines whether the pixel
// is "inside" the filled region:
//   operator Less / LessEq     : inside = (f <= 0)  → lhs <= rhs
//   operator Greater / GreaterEq: inside = (f >= 0)  → lhs >= rhs
// The output is a u32 per pixel holding premultiplied RGBA8 (white mask with
// varying alpha) that can be uploaded directly as an egui::ColorImage.
//
// Bindings:
//   0 - uniform FillParams
//   1 - storage read bytecode (u32[])
//   2 - storage read constants (f32[])
//   3 - storage read_write output (u32[])
...
// GPU y la CPU discrepan y la máscara se dibuja en otro lugar.
...
fn cs_main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let w = f32(max(params.width - 1u, 1u));
    let h = f32(max(params.height - 1u, 1u));
    // Map pixel column to world x (left → x_min, right → x_max).
    let x = params.x_min + f32(gid.x) * (params.x_max - params.x_min) / w;
    // Flip y: pixel row 0 = top = y_max, row h-1 = bottom = y_min.
    let y = params.y_max - f32(gid.y) * (params.y_max - params.y_min) / h;

    // Evaluate f(x, y) = lhs - rhs.
    let f = eval_bytecode(x, y);
    ...
    if params.op_code <= 1u {
        ...
        // Greater (2) / GreaterEq (3): inside = (rhs - lhs) <= 0 → f >= 0
        inside = f >= 0.0;
    }

...
fn glyph_rows(ch: char) -> Option<[u8; 7]> {
    ...
    let rows = match ch {
        ...
        // Subíndices ₀-₉ (3×5 en filas 2-6): los auto-labels del documento
        // son letra + subíndice (`F₃₉`); sin estos glifos cada label caía al
        // camino ASCII con teselado por frame (~50 µs/label). T1.
        '₀' => [0x00, 0x00, 0x06, 0x0A, 0x0A, 0x0A, 0x06],
        '₁' => [0x00, 0x00, 0x04, 0x04, 0x04, 0x04, 0x0E],
        '₂' => [0x00, 0x00, 0x0E, 0x02, 0x0E, 0x08, 0x0E],
        '₃' => [0x00, 0x00, 0x0E, 0x02, 0x06, 0x02, 0x0E],
        '₄' => [0x00, 0x00, 0x0A, 0x0A, 0x0E, 0x02, 0x02],
        '₅' => [0x00, 0x00, 0x0E, 0x08, 0x0E, 0x02, 0x0E],
        '₆' => [0x00, 0x00, 0x06, 0x08, 0x0E, 0x0A, 0x0E],
        '₇' => [0x00, 0x00, 0x0E, 0x02, 0x02, 0x04, 0x04],
        '₈' => [0x00, 0x00, 0x06, 0x0A, 0x06, 0x0A, 0x06],
        '₉' => [0x00, 0x00, 0x06, 0x0A, 0x0E, 0x02, 0x0C],
...
mod tests {
    ...
    #[test]
    fn digitos_subindice_cubren_autolabels() {
        // T1: los auto-labels son letra + subíndices (`F₃₉`); cada dígito
        // sin glifo mandaba el label al camino ASCII con teselado por frame.
        for ch in ['₀', '₁', '₂', '₃', '₄', '₅', '₆', '₇', '₈', '₉'] {
            assert!(has_glyph(ch), "subíndice sin glifo: {ch:?}");
        }
        let outcome = render_plain_math_to_bitmap("F₃₉ = sin(39 * x)").expect("raster válido");

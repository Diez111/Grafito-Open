...
pub fn euler_system<F>(f: F, t0: f64, y0: Vec<f64>, t_end: f64, steps: usize) -> Vec<(f64, Vec<f64>)>
where
...
/// # Returns
/// Vector of (t, [y1, y2, ...]) points representing the solution
pub fn runge_kutta_4_system<F>(f: F, t0: f64, y0: Vec<f64>, t_end: f64, steps: usize) -> Vec<(f64, Vec<f64>)>
where
    F: Fn(f64, &[f64]) -> Vec<f64>,
{
    let mut points = Vec::with_capacity(steps + 1);

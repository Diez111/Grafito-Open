...
pub fn quantile(data: &[f64], q: f64) -> Option<f64> {
    if data.is_empty() || !(0.0..=1.0).contains(&q) { return None; }
    let mut sorted = data.to_vec();
    sorted.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
...
pub fn q1(data: &[f64]) -> Option<f64> { quantile(data, 0.25) }
pub fn q3(data: &[f64]) -> Option<f64> { quantile(data, 0.75) }
pub fn iqr(data: &[f64]) -> Option<f64> { Some(q3(data)? - q1(data)?) }
...
pub fn covariance(xs: &[f64], ys: &[f64]) -> Option<f64> {
    ...
    let my = mean(ys)?;
    let n = xs.len() as f64;
    Some(xs.iter().zip(ys.iter()).map(|(x, y)| (x - mx) * (y - my)).sum::<f64>() / (n - 1.0))
}
...
pub fn polynomial_regression(xs: &[f64], ys: &[f64], degree: usize) -> Option<Vec<f64>> {
    if xs.len() != ys.len() || xs.len() <= degree { return None; }
    let m = degree + 1;
    let mut a = vec![vec![0.0f64; m + 1]; m];
    for i in 0..m {
        for j in 0..m {
            a[i][j] = xs.iter().map(|x| x.powi((i + j) as i32)).sum();
        }
        a[i][m] = xs.iter().zip(ys.iter()).map(|(x, y)| y * x.powi(i as i32)).sum();
    }
    gauss_elimination(&mut a, m)
}

fn gauss_elimination(a: &mut Vec<Vec<f64>>, n: usize) -> Option<Vec<f64>> {
    for col in 0..n {
        let mut max_row = col;
        for row in (col + 1)..n {
        ...
        a.swap(col, max_row);
        if a[col][col].abs() < 1e-15 { return None; }
        for row in (col + 1)..n {
...
pub fn exponential_regression(xs: &[f64], ys: &[f64]) -> Option<(f64, f64, f64)> {
    ...
    if log_ys.iter().any(|v| v.is_nan() || v.is_infinite()) { return None; }
...
pub fn logarithmic_regression(xs: &[f64], ys: &[f64]) -> Option<(f64, f64, f64)> {
    ...
    if log_xs.iter().any(|v| v.is_nan() || v.is_infinite()) { return None; }
...
pub fn histogram(data: &[f64], bins: usize) -> Vec<(f64, f64, f64)> {
    ...
    let width = if (hi - lo).abs() < 1e-15 { 1.0 } else { (hi - lo) / bins as f64 };
...
pub fn frequency_table(data: &[f64]) -> Vec<(f64, usize, f64, f64)> {
    ...
    let n = data.len() as f64;
    let mut table: Vec<_> = counts.values().map(|(v, c)| (*v, *c, *c as f64 / n, *c as f64 / n)).collect();
...
pub fn boxplot_stats(data: &[f64]) -> Option<(f64, f64, f64, f64, f64, Vec<f64>)> {
    ...
    let outliers: Vec<f64> = data.iter().cloned().filter(|&v| v < lower_fence || v > upper_fence).collect();
    let whisker_lo = data.iter().cloned().filter(|&v| v >= lower_fence).reduce(f64::min).unwrap_or(q1);
    let whisker_hi = data.iter().cloned().filter(|&v| v <= upper_fence).reduce(f64::max).unwrap_or(q3);
...
pub fn student_t_pdf(x: f64, nu: f64) -> f64 {
    let coeff = (gamma_ln((nu + 1.0) / 2.0) - gamma_ln(nu / 2.0) - 0.5 * (nu * std::f64::consts::PI).ln()).exp();
...
pub fn f_distribution_pdf(x: f64, d1: f64, d2: f64) -> f64 {
    ...
    let half_d1 = d1 / 2.0;
    let half_d2 = d2 / 2.0;
    let ln_coeff = half_d1 * d1.ln() + half_d2 * d2.ln() - gamma_ln(half_d1) - gamma_ln(half_d2) + gamma_ln(half_d1 + half_d2);
...
pub fn hypergeometric_pmf(n_pop: u32, k_success: u32, n_draw: u32, k_observed: u32) -> f64 {
    if k_observed > k_success || k_observed > n_draw { return 0.0; }
    if n_draw - k_observed > n_pop - k_success { return 0.0; }
    let ln_num = gamma_ln(k_success as f64 + 1.0) - gamma_ln(k_observed as f64 + 1.0) - gamma_ln((k_success - k_observed) as f64 + 1.0)
               + gamma_ln((n_pop - k_success) as f64 + 1.0) - gamma_ln((n_draw - k_observed) as f64 + 1.0) - gamma_ln((n_pop - k_success - n_draw + k_observed) as f64 + 1.0);
    let ln_den = gamma_ln(n_pop as f64 + 1.0) - gamma_ln(n_draw as f64 + 1.0) - gamma_ln((n_pop - n_draw) as f64 + 1.0);
    (ln_num - ln_den).exp()
}
...
pub fn negative_binomial_pmf(r: u32, p: f64, k: u32) -> f64 {
    let coef = super::special_functions::gamma((k + r) as f64) 
        / (super::special_functions::gamma(r as f64) * super::special_functions::gamma((k + 1) as f64));
...
pub fn chi_squared_test(observed: &[f64], expected: &[f64]) -> Option<(f64, f64)> {
    if observed.len() != expected.len() || observed.len() < 2 { return None; }
...
pub fn confidence_interval_proportion(successes: u32, n: u32, confidence: f64) -> Option<(f64, f64, f64)> {
    if n == 0 || successes > n { return None; }
...
mod tests {
    ...
    fn test_boxplot() {
        ...
        let (wl, q1, med, q3, wh, outliers) = boxplot_stats(&data).unwrap();

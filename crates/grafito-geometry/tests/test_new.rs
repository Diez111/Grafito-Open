...
fn test_atan2() {
    ...
    match parse_ast(&pp) {
        Ok(ast) => {
            ...
            eprintln!("eval_at: {}", v);
            assert!((v - std::f64::consts::FRAC_PI_4).abs() < 0.01, "got {}", v);
        }
...
fn test_new_funcs() {
    ...
    assert!(
        (grafito_geometry::expr::eval_function("besselj(0,0)", 0.0).unwrap() - 1.0).abs() < 0.01
    );
    assert!(
        (grafito_geometry::expr::eval_function("heaviside(x-2)", 3.0).unwrap() - 1.0).abs() < 0.01
    );
    assert!((grafito_geometry::expr::eval_function("min(3,5)", 0.0).unwrap() - 3.0).abs() < 0.01);
    assert!(
        (grafito_geometry::expr::eval_function("clamp(10, 0, 5)", 0.0).unwrap() - 5.0).abs() < 0.01
    );
}

...
use grafito_geometry::{Point3D, Camera3D};
use grafito_ui::Tool;
use egui::{Vec2, Stroke, Color32, Rect};
use glam::Vec3;

use crate::{GrafitoApp, to_color32};
...
impl GrafitoApp {
    pub fn handle_3d_click(&mut self, _ui: &egui::Ui, _response: &egui::Response, _canvas: Rect, w: f32, h: f32) {
        ...
        match self.current_tool {
            ...
            Tool::Sphere3D => {
                self.save_state();
                let c = Point3D::new(t.x as f64, t.y as f64, t.z as f64);
                self.document.add_object(GeoObject::Sphere3D(Sphere3DObj::new(c, approx_scale * 1.5)));
            }
            Tool::Cube3D => {
                self.save_state();
                let c = Point3D::new(t.x as f64, t.y as f64, t.z as f64);
    ...
    pub fn draw_3d_grid(&self, painter: &egui::Painter, canvas: Rect, w: f32, h: f32) {
        ...
        let target_world_step = 80.0 / pixels_per_unit.max(1e-30);
        ...
        let minor_color = if self.dark_mode {
        ...
        if self.show_grid && line_count_x <= 500 && line_count_z <= 500 {
            ...
            for zi in 0..=line_count_z {
                ...
                let idx = (z / minor_step).round() as i64;
                let is_major = idx % 5 == 0;
                let stroke = if is_major { major_stroke } else { minor_stroke };
        ...
        // X Axis
        if let Some((a, b)) = project_segment(&self.camera, &Point3D::new(-axis_len, 0.0, 0.0), &Point3D::new(axis_len, 0.0, 0.0), w, h) {
            painter.line_segment([origin + Vec2::new(a.0, a.1), origin + Vec2::new(b.0, b.1)], red_stroke);
        }
        // Y Axis (vertical)
        if let Some((a, b)) = project_segment(&self.camera, &Point3D::new(0.0, -axis_len, 0.0), &Point3D::new(0.0, axis_len, 0.0), w, h) {
            painter.line_segment([origin + Vec2::new(a.0, a.1), origin + Vec2::new(b.0, b.1)], green_stroke);
        }
        // Z Axis
        if let Some((a, b)) = project_segment(&self.camera, &Point3D::new(0.0, 0.0, -axis_len), &Point3D::new(0.0, 0.0, axis_len), w, h) {
            painter.line_segment([origin + Vec2::new(a.0, a.1), origin + Vec2::new(b.0, b.1)], blue_stroke);
        }

        // Axis labels
        let label_font = egui::FontId::proportional(14.0);
        if let Some(pos) = self.camera.project(&Point3D::new(axis_len, 0.0, 0.0), w, h) {
            painter.text(origin + Vec2::new(pos.0, pos.1) + Vec2::new(4.0, -4.0), egui::Align2::LEFT_BOTTOM, "X", label_font.clone(), red_stroke.color);
        }
        if let Some(pos) = self.camera.project(&Point3D::new(0.0, axis_len, 0.0), w, h) {
            painter.text(origin + Vec2::new(pos.0, pos.1) + Vec2::new(4.0, -4.0), egui::Align2::LEFT_BOTTOM, "Y", label_font.clone(), green_stroke.color);
        }
        if let Some(pos) = self.camera.project(&Point3D::new(0.0, 0.0, axis_len), w, h) {
            painter.text(origin + Vec2::new(pos.0, pos.1) + Vec2::new(4.0, -4.0), egui::Align2::LEFT_BOTTOM, "Z", label_font.clone(), blue_stroke.color);
        }
        ...
        if num_count_x <= 500 {
            for xi in 0..=num_count_x {
                ...
                if let Some((a, b)) = project_segment(&self.camera, &Point3D::new(x, 0.0, -tick_size), &Point3D::new(x, 0.0, tick_size), w, h) {
                    painter.line_segment([origin + Vec2::new(a.0, a.1), origin + Vec2::new(b.0, b.1)], tick_stroke);
                }
                if let Some(pos) = self.camera.project(&Point3D::new(x, 0.0, 0.0), w, h) {
                    painter.text(origin + Vec2::new(pos.0, pos.1) + Vec2::new(0.0, 6.0), egui::Align2::CENTER_TOP, format_num(x), font.clone(), text_color);
                }
            }
        }
        ...
        if num_count_y <= 500 {
            for yi in 0..=num_count_y {
                ...
                let tick_size = major_step * 0.05;
                if let Some((a, b)) = project_segment(&self.camera, &Point3D::new(-tick_size, y, 0.0), &Point3D::new(tick_size, y, 0.0), w, h) {
                    painter.line_segment([origin + Vec2::new(a.0, a.1), origin + Vec2::new(b.0, b.1)], tick_stroke);
                }
                if let Some(pos) = self.camera.project(&Point3D::new(0.0, y, 0.0), w, h) {
                    painter.text(origin + Vec2::new(pos.0, pos.1) + Vec2::new(-6.0, 0.0), egui::Align2::RIGHT_CENTER, format_num(y), font.clone(), text_color);
                }
            }
        }
        ...
        if num_count_z <= 500 {
            for zi in 0..=num_count_z {
                ...
                let tick_size = major_step * 0.05;
                if let Some((a, b)) = project_segment(&self.camera, &Point3D::new(-tick_size, 0.0, z), &Point3D::new(tick_size, 0.0, z), w, h) {
                    painter.line_segment([origin + Vec2::new(a.0, a.1), origin + Vec2::new(b.0, b.1)], tick_stroke);
                }
                if let Some(pos) = self.camera.project(&Point3D::new(0.0, 0.0, z), w, h) {
                    painter.text(origin + Vec2::new(pos.0, pos.1) + Vec2::new(8.0, 0.0), egui::Align2::LEFT_CENTER, format_num(z), font.clone(), text_color);
                }
            }
        }
        ...
        if let Some(pos) = self.camera.project(&Point3D::new(0.0, 0.0, 0.0), w, h) {
            painter.text(origin + Vec2::new(pos.0, pos.1) + Vec2::new(-6.0, 6.0), egui::Align2::RIGHT_TOP, "0", font.clone(), text_color);
        }
    }
    ...
    pub fn draw_3d_objects(&mut self, painter: &egui::Painter, canvas: Rect, w: f32, h: f32) {
        ...
        for (_, obj) in self.document.objects_iter() {
            ...
            let distance = match obj {
                GeoObject::Point3D(p) => {
                    (p.position.to_vec3() - camera_pos).length()
                }
                ...
                GeoObject::Sphere3D(s) => {
                    (s.center.to_vec3() - camera_pos).length()
                }
                GeoObject::Cube3D(c) => {
                    (c.center.to_vec3() - camera_pos).length()
                }
                ...
                GeoObject::Torus3D(t) => {
                    (t.center.to_vec3() - camera_pos).length()
                }
                GeoObject::MoebiusStrip(m) => {
                    (m.center.to_vec3() - camera_pos).length()
                }
        ...
        objects_with_depth.sort_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
            ...
            match obj {
                GeoObject::Point3D(p) => {
                    if let Some(pt) = self.camera.project(&p.position, w, h) {
                        ...
                        if !p.label.is_empty() {
                            painter.text(pos + Vec2::new(6.0, -6.0), egui::Align2::LEFT_BOTTOM, &p.label, egui::FontId::proportional(12.0), label_color);
                        }
                    }
                }
                GeoObject::Segment3D(l) => {
                    if let Some((a, b)) = project_segment(&self.camera, &l.a, &l.b, w, h) {
                        ...
                        if !l.label.is_empty() {
                            ...
                            painter.text(origin + Vec2::new(mid, mid_y - 8.0), egui::Align2::CENTER_BOTTOM, &l.label, egui::FontId::proportional(12.0), label_color);
                        }
                    }
                }
                GeoObject::Sphere3D(s) => {
                    ...
                    for &(u, v) in &axes {
                        ...
                        let pts_screen: Vec<Option<(f32, f32)>> = pts_3d.iter()
                        ...
                        for i in 0..pts_3d.len() {
                            ...
                            if let (Some(p1), Some(p2)) = (pts_screen[i], pts_screen[i2]) {
                                ...
                                // Apply lighting
                                let lit_color = grafito_render::calculate_lighting(s.color, normal, light_dir);
                    ...
                    if !s.label.is_empty() {
                        if let Some(pt) = self.camera.project(&Point3D::new(s.center.x, s.center.y + s.radius + 0.3, s.center.z), w, h) {
                            painter.text(origin + Vec2::new(pt.0, pt.1), egui::Align2::CENTER_BOTTOM, &s.label, egui::FontId::proportional(12.0), label_color);
                        }
                    }
                }
                GeoObject::Cube3D(cube) => {
                    ...
                    for &((a, b), normal) in &edges_with_normals {
                        if let (Some(pa), Some(pb)) = (self.camera.project(&vs[a], w, h), self.camera.project(&vs[b], w, h)) {
                            let lit_color = grafito_render::calculate_lighting(cube.color, normal, light_dir);
                    ...
                    if !cube.label.is_empty() {
                        if let Some(pt) = self.camera.project(&Point3D::new(cube.center.x, cube.center.y + cube.size * 0.7, cube.center.z), w, h) {
                            painter.text(origin + Vec2::new(pt.0, pt.1), egui::Align2::CENTER_BOTTOM, &cube.label, egui::FontId::proportional(12.0), label_color);
                        }
                    }
                }
                GeoObject::Pyramid3D(py) => {
                    ...
                    for i in 0..4 {
                        ...
                        if let (Some(a), Some(b)) = (a_proj, b_proj) {
                            let lit_color = grafito_render::calculate_lighting(py.color, base_normal, light_dir);
                    ...
                    for i in 0..4 {
                        ...
                        if let (Some(a), Some(ap)) = (a_proj, apex_proj) {
                            let lit_color = grafito_render::calculate_lighting(py.color, face_normal, light_dir);
                            let stroke = Stroke::new(py.width, to_color32(lit_color));
                    ...
                    if !py.label.is_empty() {
                        if let Some(pt) = self.camera.project(&py.apex, w, h) {
                            painter.text(origin + Vec2::new(pt.0, pt.1 + 14.0), egui::Align2::CENTER_TOP, &py.label, egui::FontId::proportional(12.0), label_color);
                        }
                    }
                }
                GeoObject::Cone3D(cone) => {
                    ...
                    let base_pts: Vec<(f32, f32)> = base_pts_3d.iter()
                        .filter_map(|pt| self.camera.project(&Point3D::from_vec3(*pt), w, h))
                        .collect();
                    
                    for i in 0..base_pts.len() {
                        let j = (i + 1) % base_pts.len();
                        let lit_color = grafito_render::calculate_lighting(cone.color, base_normal, light_dir);
                    ...
                    if let Some(ap) = self.camera.project(&cone.apex, w, h) {
                        for i in 0..base_pts_3d.len() {
                            let bp_3d = base_pts_3d[i];
                            if let Some(bp) = self.camera.project(&Point3D::from_vec3(bp_3d), w, h) {
                                ...
                                let axial = (cone.apex.to_vec3() - cone.base_center.to_vec3()).normalize();
                                let lateral_normal = (radial + axial * 0.5).normalize();
                                
                                let lit_color = grafito_render::calculate_lighting(cone.color, lateral_normal, light_dir);
                                let stroke = Stroke::new(cone.width, to_color32(lit_color));
                    ...
                    if !cone.label.is_empty() {
                        if let Some(pt) = self.camera.project(&cone.apex, w, h) {
                            painter.text(origin + Vec2::new(pt.0, pt.1 + 14.0), egui::Align2::CENTER_TOP, &cone.label, egui::FontId::proportional(12.0), label_color);
                        }
                    }
                }
                GeoObject::Cylinder3D(cyl) => {
                    ...
                    for &(center, normal) in &circles {
                        ...
                        let pts: Vec<(f32, f32)> = pts_3d.iter()
                            .filter_map(|pt| self.camera.project(&Point3D::from_vec3(*pt), w, h))
                            .collect();
                        
                        for i in 0..pts.len() {
                            let j = (i + 1) % pts.len();
                            let lit_color = grafito_render::calculate_lighting(cyl.color, normal, light_dir);
                    ...
                    if let (Some(_a), Some(_b)) = (self.camera.project(&cyl.base_center, w, h), self.camera.project(&cyl.top_center, w, h)) {
                        for angle in [0.0, std::f32::consts::PI * 0.5, std::f32::consts::PI, std::f32::consts::PI * 1.5] {
                            ...
                            if let (Some(ca), Some(cb)) = (ca, cb) {
                                let lit_color = grafito_render::calculate_lighting(cyl.color, radial_normal, light_dir);
                                let stroke = Stroke::new(cyl.width, to_color32(lit_color));
                    ...
                    if !cyl.label.is_empty() {
                        if let Some(pt) = self.camera.project(&Point3D::new(cyl.top_center.x, cyl.top_center.y + 0.5, cyl.top_center.z), w, h) {
                            painter.text(origin + Vec2::new(pt.0, pt.1), egui::Align2::CENTER_BOTTOM, &cyl.label, egui::FontId::proportional(12.0), label_color);
                        }
                    }
                }
                GeoObject::Torus3D(torus) => {
                    let stroke = Stroke::new(torus.width, to_color32(torus.color));
                    let steps_u = 24; let steps_v = 12;
                    let r_maj = torus.r_major; let r = torus.r_minor;
                    ...
                    for i in 0..=steps_u {
                        ...
                        for j in 0..=steps_v {
                            ...
                            if let Some(p) = self.camera.project(&Point3D::new(x, y, z), w, h) {
                                pts[i][j] = (p.0, p.1); valid[i][j] = true;
                            }
                        }
                    }
                    for i in 0..steps_u {
                        for j in 0..steps_v {
                            if valid[i][j] && valid[i+1][j] {
                                painter.line_segment([origin + Vec2::new(pts[i][j].0, pts[i][j].1), origin + Vec2::new(pts[i+1][j].0, pts[i+1][j].1)], stroke);
                            }
                            if valid[i][j] && valid[i][j+1] {
                                painter.line_segment([origin + Vec2::new(pts[i][j].0, pts[i][j].1), origin + Vec2::new(pts[i][j+1].0, pts[i][j+1].1)], stroke);
                            }
                        }
                    }
                }
                GeoObject::MoebiusStrip(moe) => {
                    ...
                    let steps_u = 32; let steps_v = 8;
                    let r_maj = moe.radius; let w_max = moe.width_r;
                    let mut pts = vec![vec![(0.0_f32, 0.0_f32); steps_v + 1]; steps_u + 1];
                    let mut valid = vec![vec![false; steps_v + 1]; steps_u + 1];
                    for i in 0..=steps_u {
                        let u = (i as f64) * std::f64::consts::PI * 2.0 / (steps_u as f64);
                ...
                GeoObject::Surface3D(surf) => {
                    ...
                    if let Ok(z_vals) = grafito_geometry::expr::eval_surface_batch(&surf.expr, pts_x.iter().copied(), &self.document.variables) {
                        for i in 0..=steps {
                    ...
                    if let Ok(z_vals) = grafito_geometry::expr::eval_surface_batch(&surf.expr, pts_y.iter().copied(), &self.document.variables) {
                        for j in 0..=steps {
                            let mut prev: Option<(f32, f32)> = None;
                            for i in 0..=steps {
                                let idx = j * (steps + 1) + i;
                ...
                GeoObject::ParametricCurve3D(curve) => {
                    ...
                    let xs = grafito_geometry::expr::eval_batch_1d(&curve.expr_x, "t", ts.clone(), variables).unwrap_or_default();
                    let ys = grafito_geometry::expr::eval_batch_1d(&curve.expr_y, "t", ts.clone(), variables).unwrap_or_default();
                    let zs = grafito_geometry::expr::eval_batch_1d(&curve.expr_z, "t", ts.clone(), variables).unwrap_or_default();
                    
                    let mut prev: Option<(f32, f32)> = None;
                    for i in 0..=steps {
                        if let (Some(Some(x)), Some(Some(y)), Some(Some(z))) = (xs.get(i), ys.get(i), zs.get(i)) {
                            if let Some(pt) = self.camera.project(&Point3D::new(*x, *z, *y), w, h) {
                                if let Some(pp) = prev {
                                    painter.line_segment([origin + Vec2::new(pp.0, pp.1), origin + Vec2::new(pt.0, pt.1)], stroke);
                                }
                ...
                GeoObject::Attractor3D(att) => {
                    use grafito_geometry::attractors::{AttractorType, integrate_attractor};
                    ...
                    let pts = if let Some((cached_hash, cached_pts)) = self.attractor_cache.get(&att.id) {
                        if *cached_hash == param_hash {
                            ...
                            let atype = match att.attractor_type.as_str() {
                                ...
                                "thomas" => {
                                    AttractorType::Thomas { b: att.params.first().copied().unwrap_or(0.208186) }
                                }
                                "aizawa" => {
                                    let p = &att.params;
                                ...
                                "halvorsen" => {
                                    AttractorType::Halvorsen { a: att.params.first().copied().unwrap_or(1.89) }
                                }
                                "dadras" => {
                                    let p = &att.params;
                            ...
                            let new_pts = integrate_attractor(&atype, att.x0, att.y0, att.z0, att.dt, att.steps, att.skip);
                            self.attractor_cache.insert(att.id, (param_hash, new_pts.clone()));
                            new_pts
                        }
                    } else {
                        let atype = match att.attractor_type.as_str() {
                    ...
                    for pt in &pts {
                        ...
                        if let Some(sp) = self.camera.project(&scaled_pt, w, h) {
                            if let Some(pp) = prev {
                                painter.line_segment([origin + Vec2::new(pp.0, pp.1), origin + Vec2::new(sp.0, sp.1)], stroke);
                            }
                            prev = Some(sp);
                        } else {
                            prev = None;
                    ...
                    if !att.label.is_empty() {
                        if let Some(first) = pts.first() {
                            if let Some(pt) = self.camera.project(first, w, h) {
                                painter.text(origin + Vec2::new(pt.0, pt.1 - 10.0), egui::Align2::CENTER_BOTTOM, &att.label, egui::FontId::proportional(12.0), label_color);
                            }
                        }
                    }
                }
                GeoObject::HyperSurface4D(hs) => {
                    ...
                    match hs.surface_type.as_str() {
                        "hypercube" => {
                            ...
                            let cos_xy = a_xy.cos(); let sin_xy = a_xy.sin();
                            let cos_xz = a_xz.cos(); let sin_xz = a_xz.sin();
                            let cos_xw = a_xw.cos(); let sin_xw = a_xw.sin();
                            let projected: Vec<Point3D> = verts_4d.iter().map(|v| {
                                let mut p = *v;
                            ...
                            let edges: Vec<(usize, usize)> = (0..16usize).flat_map(|i| {
                                (0..16usize).filter(move |&j| j > i && (i ^ j).count_ones() == 1).map(move |j| (i, j))
                            }).collect();
                            for &(a, b) in &edges {
                                if let (Some(pa), Some(pb)) = (self.camera.project(&projected[a], w, h), self.camera.project(&projected[b], w, h)) {
                                    painter.line_segment([origin + Vec2::new(pa.0, pa.1), origin + Vec2::new(pb.0, pb.1)], stroke);
                                }
                            }
                            if !hs.label.is_empty() {
                                if let Some(pt) = self.camera.project(&projected[0], w, h) {
                                    painter.text(origin + Vec2::new(pt.0, pt.1 - 10.0), egui::Align2::CENTER_BOTTOM, &hs.label, egui::FontId::proportional(12.0), label_color);
                                }
                            }
                        }
                        "hypersphere" => {
                            let res = hs.resolution.max(8).min(30);
                            ...
                            for i in 0..=res {
                                ...
                                for j in 0..=res * 2 {
                                    ...
                                    p[0] = nx2; p[3] = nw;
                                    let w_factor = 1.0 / (3.0 - p[3] / scale);
                                    ring.push(Point3D::new(p[0] * w_factor, p[1] * w_factor, p[2] * w_factor));
                                }
                                pts_3d.push(ring);
                            }
                            for ring in &pts_3d {
                    ...
                    for i in 0..=n {
                        for j in 0..=n {
                            for k in 0..=n {
                                ...
                                let u = grafito_geometry::expr::evaluate(&vf.expr_u, &local_vars).unwrap_or(0.0);
                                let v = grafito_geometry::expr::evaluate(&vf.expr_v, &local_vars).unwrap_or(0.0);
                                let w_val = grafito_geometry::expr::evaluate(&vf.expr_w, &local_vars).unwrap_or(0.0);
                                if !u.is_finite() || !v.is_finite() || !w_val.is_finite() { continue; }
                                ...
                                if let (Some(pa), Some(pb)) = (self.camera.project(&start, w, h), self.camera.project(&end, w, h)) {
                                    painter.line_segment([origin + Vec2::new(pa.0, pa.1), origin + Vec2::new(pb.0, pb.1)], stroke);
                                }
                            }
                        }

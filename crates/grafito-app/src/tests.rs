...
#[test]
fn value_table_range_rejects_non_positive_step() {
    let err = crate::panels::validate_value_table_range("0", "10", "0")
        .expect_err("zero step must be rejected");
    assert!(err.contains("Paso"));

    let err = crate::panels::validate_value_table_range("0", "10", "-1")
        .expect_err("negative step must be rejected");
    assert!(err.contains("Paso"));
}

#[test]
fn value_table_range_caps_generated_points() {
    let err = crate::panels::validate_value_table_range("0", "100000", "0.001")
        .expect_err("huge ranges must be rejected before creating points");
    assert!(err.contains("demasiados puntos"));

    let (_, _, _, count) = crate::panels::validate_value_table_range("0", "1", "0.25")
        .expect("small valid range should pass");
    assert_eq!(count, 5);
}

#[test]
fn value_table_display_is_bounded_for_valid_large_ranges() {
    assert_eq!(crate::panels::value_table_display_count(42), 42);
    assert_eq!(crate::panels::value_table_display_count(10_000), 500);
}
...
#[test]
fn value_table_generation_is_atomic_when_only_one_object_slot_remains() {
    use grafito_core::ChangeSet;
    ...
    let removed = document
        ...
        .next()
        .map(|(id, _)| *id)
        .expect("capacity fixture is not empty");
    assert!(document.remove_object(removed).is_some());
    let before = serde_json::to_value(&document).expect("serialize nearly full document");
    let mut undo_stack = Vec::new();
    let mut redo_stack = vec![ChangeSet {
        before: grafito_core::Document::new(),
        after: grafito_core::Document::new(),
    ...
    let error = crate::panels::commit_value_table_points(
        &mut document,
        &mut undo_stack,
        &mut redo_stack,
        crate::panels::ValueTablePointSpec {
            expression: "x",
            variable: "x",
            x_min: 0.0,
            step: 1.0,
    ...
    .expect_err("the complete two-point generation must be rejected");
    ...
    assert_eq!(
        document.object_count(),
        grafito_core::validation::MAX_OBJECT_COUNT - 1
    );
    assert_eq!(
        serde_json::to_value(&document).expect("serialize rejected generation"),
        before
    );
}

#[test]
fn successful_value_table_generation_creates_one_undo_snapshot() {
    use grafito_core::ChangeSet;
    ...
    let inserted = crate::panels::commit_value_table_points(
        &mut document,
        &mut undo_stack,
        &mut redo_stack,
        crate::panels::ValueTablePointSpec {
    ...
    .expect("three valid table points must commit");

    assert_eq!(inserted, 3);
    assert_eq!(document.object_count(), 3);
    assert_eq!(undo_stack.len(), 1);
    assert_eq!(undo_stack[0].object_count(), 0);
    assert!(redo_stack.is_empty());
}

#[test]
fn spreadsheet_column_labels_cover_multi_letter_columns_without_u8_overflow() {
    assert_eq!(crate::panels::spreadsheet_column_label(0), "A");
    assert_eq!(crate::panels::spreadsheet_column_label(25), "Z");
    assert_eq!(crate::panels::spreadsheet_column_label(26), "AA");
    assert_eq!(crate::panels::spreadsheet_column_label(191), "GJ");
    assert_eq!(crate::panels::spreadsheet_column_label(1_000), "ALM");
}

#[test]
fn spreadsheet_visible_window_is_bounded_for_a_persisted_million_cell_sheet() {
    let viewport = egui::Rect::from_min_size(egui::Pos2::ZERO, egui::vec2(260.0, 180.0));
    let (rows, columns) = crate::panels::spreadsheet_visible_window(viewport, 1_000, 1_000);

    assert!(rows.len() * columns.len() <= 200);
    assert!(rows.end <= 1_000);
    assert!(columns.end <= 1_000);

    let scrolled = egui::Rect::from_min_size(
        egui::pos2(52.0 + 400.0 * 60.0, 24.0 + 700.0 * 24.0),
        egui::vec2(260.0, 180.0),
    );
    let (rows, columns) = crate::panels::spreadsheet_visible_window(scrolled, 1_000, 1_000);
    assert!(rows.contains(&700));
    assert!(columns.contains(&400));
}

#[test]
fn spreadsheet_coordinate_cell_rejects_a_manual_label_collision_atomically() {
    use grafito_core::GeoObject;

    let mut document = grafito_core::Document::new();
    let manual = document.add_object(GeoObject::Point(
        grafito_core::PointObj::new(grafito_geometry::Point2::new(9.0, 9.0)).with_label("A1"),
    ));
    let before = serde_json::to_value(&document).expect("serialize initial document");

    let error =
        crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(1, 2)".to_string())
            .expect_err("a coordinate cell cannot duplicate a manual object label");

    assert!(error.contains("A1"));
    assert_eq!(document.spreadsheet_coordinate_point("A1"), None);
    assert!(document.get_object(manual).is_some());
    assert_eq!(
        serde_json::to_value(&document).expect("serialize final document"),
        before
    );
}

#[test]
fn spreadsheet_coordinate_cell_recreates_its_owner_after_external_point_removal() {
    let mut document = grafito_core::Document::new();
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(1, 2)".to_string())
        .expect("set coordinate cell");
    let removed = document
        .spreadsheet_coordinate_point("A1")
        .expect("coordinate cell owns its generated point");

    assert!(document.remove_object(removed).is_some());
    assert_eq!(document.spreadsheet_coordinate_point("A1"), None);

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(3, 4)".to_string())
        .expect("edit coordinate cell");
    let replacement = document
        .spreadsheet_coordinate_point("A1")
        .expect("coordinate cell recreates its removed point");
    assert_ne!(replacement, removed);
    assert!(matches!(
        document.get_object(replacement),
        Some(grafito_core::GeoObject::Point(point))
            if point.position == grafito_geometry::Point2::new(3.0, 4.0)
    ));
}

#[test]
fn spreadsheet_coordinate_edit_rolls_back_cell_and_geometry_when_numeric_solving_fails() {
    use grafito_core::{GeoObject, PointObj};
    use grafito_geometry::Point2;

    let mut document = grafito_core::Document::new();
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(0, 0)".to_string())
        .expect("create spreadsheet coordinate point");
    let spreadsheet_point = document
        .spreadsheet_coordinate_point("A1")
        .expect("spreadsheet point exists");
    let other_point = document.add_object(GeoObject::Point(PointObj::new(Point2::new(1.0, 0.0))));
    let (midpoint, _) = document.add_constructed_object(
        GeoObject::Point(PointObj::new(Point2::new(0.0, 0.0))),
        "Midpoint",
        &[spreadsheet_point, other_point],
    );
    let order = document.propagation_order(&[spreadsheet_point, other_point]);
    document
        .try_re_evaluate_constraints(&order)
        .expect("initial midpoint must evaluate");
    document
        .try_add_distance_constraint(spreadsheet_point, other_point, 1.0)
        .expect("first distance is valid");
    document
        .try_add_distance_constraint(spreadsheet_point, other_point, 2.0)
        .expect("conflicting distance can be staged for this regression");

    let before = serde_json::to_value(&document).expect("document should serialize");
    let version_before = document.version;
    let midpoint_before = document
        .point_position(midpoint)
        .expect("midpoint should remain available");

    let error =
        crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(4, 0)".to_string())
            .expect_err("incompatible numeric constraints must reject the coordinate edit");

    assert!(error.contains("Numeric constraint"));
    assert_eq!(document.get_spreadsheet_cell(0, 0), "(0, 0)");
    assert_eq!(document.version, version_before);
    assert_eq!(document.point_position(midpoint), Some(midpoint_before));
    assert_eq!(
...
#[test]
fn spreadsheet_coordinate_text_stays_staged_until_one_committed_undoable_edit() {
    use grafito_core::{GeoObject, PointObj};
    use grafito_geometry::Point2;
    use std::collections::HashMap;
    ...
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(1, 2)".to_string())
        .expect("create spreadsheet coordinate point");
    let coordinate_point = document
        .spreadsheet_coordinate_point("A1")
        .expect("coordinate cell owns a point");
    let other_point = document.add_object(GeoObject::Point(PointObj::new(Point2::new(5.0, 2.0))));
    let (midpoint, _) = document
        .try_add_constructed_object(
            GeoObject::Point(PointObj::new(Point2::new(0.0, 0.0)).with_label("M")),
            "Midpoint",
            &[coordinate_point, other_point],
        )
        .expect("midpoint construction should be valid");
    let order = document.propagation_order(&[coordinate_point, other_point]);
    document
        .try_re_evaluate_constraints(&order)
        .expect("initial midpoint should evaluate");
    ...
    .expect("in-progress text must remain local"));
    assert_eq!(document.version, version_before);
    assert_eq!(
        serde_json::to_value(&document).expect("document should serialize"),
        before
    );
    assert_eq!(edit_buffers.get(&(0, 0)), Some(&"(3,".to_string()));
    assert!(undo_stack.is_empty());
    assert_eq!(redo_stack.len(), 1);
    ...
    .expect("incomplete coordinate text must remain staged after focus loss"));
    assert_eq!(document.version, version_before);
    assert_eq!(
        serde_json::to_value(&document).expect("document should serialize"),
        before
    ...
    .expect("completed coordinate edit should commit"));
    assert_eq!(
        document.point_position(coordinate_point),
        Some(Point2::new(3.0, 4.0))
    );
    assert_eq!(
        document.point_position(midpoint),
        Some(Point2::new(4.0, 3.0))
    );
    assert_eq!(undo_stack.len(), 1);
    assert!(redo_stack.is_empty());
...
#[test]
fn spreadsheet_draft_edits_remain_local_until_one_committed_snapshot() {
    use std::collections::HashMap;
    ...
    .expect("draft edit should remain local"));
    assert_eq!(document.version, version_before);
    assert_eq!(
        serde_json::to_value(&document).expect("document should serialize"),
        before
    );
    assert_eq!(edit_buffers.get(&(0, 0)), Some(&"2".to_string()));
    assert!(undo_stack.is_empty());
    assert_eq!(redo_stack.len(), 1);
    ...
    .expect("committed edit should mutate once"));
    assert_eq!(document.get_spreadsheet_cell(0, 0), "2");
    assert!(edit_buffers.is_empty());
    assert_eq!(undo_stack.len(), 1);
    assert!(redo_stack.is_empty());
}
...
fn spreadsheet_commit_recomputes_sources_and_records_one_undo() {
    ...
    assert_eq!(undo_stack.len(), 1);
    assert_eq!(
        serde_json::to_value(&undo_stack[0]).expect("undo document should serialize"),
        before
    );
    assert!(redo_stack.is_empty());
}
...
#[test]
fn mini_spreadsheet_cell_edits_use_coordinate_reconciliation() {
    let mut document = grafito_core::Document::new();

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(1, 2)".to_string())
        .expect("set coordinate cell through shared edit path");
    let point = document
        .spreadsheet_coordinate_point("A1")
        .expect("mini sheet coordinate cell owns a point");
    assert_eq!(document.get_spreadsheet_cell(0, 0), "(1, 2)");

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, String::new())
        .expect("clear coordinate cell through shared edit path");
    assert!(document.get_object(point).is_none());
    assert_eq!(document.spreadsheet_coordinate_point("A1"), None);
}

#[test]
fn mini_spreadsheet_numeric_cell_set_and_clear_reconciles_its_variable() {
    let mut document = grafito_core::Document::new();
    document.set_variable("manual".to_string(), 7.0);

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "42".to_string())
        .expect("set mini numeric cell");
    assert_eq!(document.get_spreadsheet_cell(0, 0), "42");
    assert_eq!(document.get_variable("A1"), Some(42.0));
    assert_eq!(document.get_variable("manual"), Some(7.0));

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, String::new())
        .expect("clear mini numeric cell");
    assert_eq!(document.get_variable("A1"), None);
    assert_eq!(document.get_variable("manual"), Some(7.0));
}

#[test]
fn full_spreadsheet_formula_edits_update_and_invalidation_removes_its_variable() {
    let mut document = grafito_core::Document::new();
    document.set_variable("manual".to_string(), 7.0);

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "2 + 3".to_string())
        .expect("set full-sheet formula");
    assert_eq!(document.get_spreadsheet_cell(0, 1), "2 + 3");
    assert_eq!(document.get_variable("B1"), Some(5.0));

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "2 * 4".to_string())
        .expect("update full-sheet formula");
    assert_eq!(document.get_variable("B1"), Some(8.0));

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, String::new())
        .expect("clear full-sheet formula");
    assert_eq!(document.get_variable("B1"), None);

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "2 * 4".to_string())
        .expect("restore full-sheet formula");
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "(".to_string())
        .expect("store invalid full-sheet formula");
    assert_eq!(document.get_variable("B1"), None);
    assert_eq!(document.get_variable("manual"), Some(7.0));
}

#[test]
fn mini_and_full_spreadsheets_share_variable_and_coordinate_reconciliation() {
    let mut mini_document = grafito_core::Document::new();
    let mut full_document = grafito_core::Document::new();

    for document in [&mut mini_document, &mut full_document] {
        crate::panels::apply_spreadsheet_cell_edit(document, 1, 0, "3 * 3".to_string())
            .expect("set formula through spreadsheet view");
        crate::panels::apply_spreadsheet_cell_edit(document, 0, 0, "(1, 2)".to_string())
            .expect("set coordinate through spreadsheet view");
    }

    assert_eq!(mini_document.get_spreadsheet_cell(1, 0), "3 * 3");
    assert_eq!(mini_document.get_variable("A2"), Some(9.0));
    assert_eq!(full_document.get_spreadsheet_cell(1, 0), "3 * 3");
    assert_eq!(full_document.get_variable("A2"), Some(9.0));

    for document in [&mut mini_document, &mut full_document] {
        let point = document
            .spreadsheet_coordinate_point("A1")
            .expect("coordinate owner exists in spreadsheet view");
        assert!(matches!(
            document.get_object(point),
            Some(grafito_core::GeoObject::Point(point))
                if point.position == grafito_geometry::Point2::new(1.0, 2.0)
        ));
        crate::panels::apply_spreadsheet_cell_edit(document, 0, 0, String::new())
            .expect("clear coordinate through spreadsheet view");
        assert!(document.get_object(point).is_none());
    }
}

#[test]
fn spreadsheet_dependencies_recompute_after_shared_mini_or_full_edits() {
    for document in [
        &mut grafito_core::Document::new(),
        &mut grafito_core::Document::new(),
    ] {
        document.set_variable("manual".to_string(), 7.0);

        crate::panels::apply_spreadsheet_cell_edit(document, 0, 0, "1".to_string())
            .expect("set A1");
        crate::panels::apply_spreadsheet_cell_edit(document, 0, 1, "A1 + 1".to_string())
            .expect("set B1");
        assert_eq!(document.get_variable("B1"), Some(2.0));

        crate::panels::apply_spreadsheet_cell_edit(document, 0, 0, "2".to_string())
            .expect("update A1");
        assert_eq!(document.get_variable("A1"), Some(2.0));
        assert_eq!(document.get_variable("B1"), Some(3.0));
...
#[test]
fn spreadsheet_dependency_cycles_and_invalid_cells_leave_no_stale_variables() {
    let mut document = grafito_core::Document::new();
    document.set_variable("manual".to_string(), 7.0);
    ...
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "A1 + 1".to_string())
        .expect("set B1");
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(".to_string())
        .expect("store invalid A1");
    assert_eq!(document.get_variable("A1"), None);
    assert_eq!(document.get_variable("B1"), None);

    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "B1 + 1".to_string())
        .expect("store cyclic A1");
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "A1 + 1".to_string())
        .expect("store cyclic B1");
    assert_eq!(document.get_variable("A1"), None);
    assert_eq!(document.get_variable("B1"), None);
    assert_eq!(document.get_variable("manual"), Some(7.0));
}
...
#[test]
fn spreadsheet_recomputation_has_a_fixed_cell_limit_and_clears_stale_values() {
    let mut document = grafito_core::Document::new();
    document.set_variable("manual".to_string(), 7.0);
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "1".to_string())
        .expect("set initial spreadsheet value");

    for index in 0..=grafito_core::Document::MAX_SPREADSHEET_RECOMPUTE_CELLS {
        let row = index / grafito_core::Document::MAX_SPREADSHEET_COLS;
        let column = index % grafito_core::Document::MAX_SPREADSHEET_COLS;
        document
            .set_spreadsheet_cell(row, column, "1".to_string())
            .expect("populate bounded spreadsheet");
    }

    let error = document
        .recompute_spreadsheet_variables()
        .expect_err("too many active cells must not trigger unbounded recomputation");
    assert!(error.contains("recomputation limit"));
    assert_eq!(document.get_variable("A1"), None);
    assert_eq!(document.get_variable("manual"), Some(7.0));
}
...
#[test]
fn regression_drawer_is_not_the_spreadsheet_editor() {
    assert!(crate::RightPanelContent::Data.uses_spreadsheet_editor());
    assert!(crate::RightPanelContent::Spreadsheet.uses_spreadsheet_editor());
    assert!(!crate::RightPanelContent::Regression.uses_spreadsheet_editor());

    let source = include_str!("app.rs");
    assert!(source.contains("draw_right_regression_panel(self, ctx)"));
}
...
fn idle_object_panel_edit_does_not_change_document_version() {
    ...
    for (start_marker, end_marker) in [
        (
            ...
            "pub(crate) fn draw_right_table_panel",
        ),
        (
        ...
        assert!(!panel.contains("get_object_mut"));
...
fn object_panel_edit_uses_staged_replacement_and_rejection_preserves_history() {
    ...
    let source = include_str!("panels.rs");
    let start = source
        .find("pub(crate) fn apply_object_panel_edit")
        .expect("shared panel edit helper");
    let end = source[start..]
        .find("pub(crate) fn validate_value_table_range")
        .map(|offset| start + offset)
        .expect("next helper");
    let helper = &source[start..end];
    assert!(helper.contains("try_replace_object"));
    assert!(!helper.contains("get_object_mut"));
}

#[test]
fn compound_regular_polytope_panel_edits_commit_once_and_reset_nd_rotations() {
    use grafito_core::{GeoObject, RegularPolychoron4DObj, RegularPolytopeNDObj};
...
fn geometry_3d_polytope_inspectors_expose_labeled_scrollable_controls() {
    ...
    let inspector_end = source[inspector_start..]
        .find("pub(crate) fn draw_right_table_panel")
...
#[test]
fn spreadsheet_draft_dirty_check_does_not_commit_editor_state() {
    let document = crate::app::initial_document();
    let lifecycle = crate::app::DocumentLifecycle::new(&document);
    let before = serde_json::to_value(&document).expect("document should serialize");
    let mut drafts = std::collections::HashMap::from([((0, 0), "42".to_string())]);

    assert!(crate::app::spreadsheet_drafts_are_dirty(&document, &drafts));
    assert!(lifecycle.is_dirty(&document, true));
    assert_eq!(document.get_spreadsheet_cell(0, 0), "");
    assert_eq!(
        serde_json::to_value(&document).expect("document should serialize"),
        before
    );

    drafts.insert((0, 0), String::new());
    assert!(!crate::app::spreadsheet_drafts_are_dirty(
        &document, &drafts
    ));
    assert!(!lifecycle.is_dirty(&document, false));

    let mut populated = crate::app::initial_document();
    crate::panels::apply_spreadsheet_cell_edit(&mut populated, 0, 0, "42".to_string())
        .expect("populate cell");
    assert!(crate::app::spreadsheet_drafts_are_dirty(
        &populated,
        &std::collections::HashMap::from([((0, 0), String::new())])
    ));
}

#[test]
fn valid_and_invalid_spreadsheet_drafts_gate_every_destructive_action() {
    use crate::app::{DocumentAction, DocumentActionRequest, DocumentLifecycle};

    for document_is_dirty in [false, true] {
        for draft in ["7", "(3,"] {
            for action in [
                DocumentAction::New,
                DocumentAction::Open,
                ...
                let drafts = std::collections::HashMap::from([((0, 0), draft.to_string())]);
                let has_drafts = crate::app::spreadsheet_drafts_are_dirty(&document, &drafts);

                assert_eq!(
                    lifecycle.request_action(action, &document, has_drafts),
                    DocumentActionRequest::AwaitDecision(action)
                );
                assert_eq!(lifecycle.pending_action(), Some(action));
                assert_eq!(document.get_spreadsheet_cell(0, 0), "");
                assert_eq!(drafts.get(&(0, 0)).map(String::as_str), Some(draft));
            }
        }
    }
}
...
#[test]
fn spreadsheet_draft_save_writes_staged_state_before_one_live_commit() {
    let mut document = crate::app::initial_document();
    let before = document.clone();
    let mut lifecycle = crate::app::DocumentLifecycle::new(&document);
    let mut drafts =
        std::collections::HashMap::from([((0, 0), "2".to_string()), ((0, 1), "5".to_string())]);
    let mut undo_stack = Vec::new();
    let mut redo_stack = vec![grafito_core::ChangeSet {
        before: document.clone(),
        after: document.clone(),
    ...
    let changed = crate::app::save_document_with_spreadsheet_drafts(
        &mut document,
        &mut drafts,
        &mut undo_stack,
        &mut redo_stack,
        |staged| {
            assert_eq!(staged.get_spreadsheet_cell(0, 0), "2");
            assert_eq!(staged.get_spreadsheet_cell(0, 1), "5");
            written.replace(Some(
                serde_json::to_value(staged).expect("staged document should serialize"),
            ));
            Ok(())
        },
    )
    .expect("valid drafts should save atomically");

    assert!(changed);
    assert_eq!(document.get_spreadsheet_cell(0, 0), "2");
    assert_eq!(document.get_spreadsheet_cell(0, 1), "5");
    assert_eq!(
        written.into_inner(),
        Some(serde_json::to_value(&document).expect("live document should serialize"))
    );
    ...
    assert!(redo_stack.is_empty());
    assert!(lifecycle.is_dirty(&document, false));
    lifecycle.record_save_success(
        std::path::PathBuf::from("/tmp/grafito-draft-save.json"),
        &document,
    );
    assert!(!lifecycle.is_dirty(&document, false));
}
...
fn spreadsheet_draft_save_uses_the_final_batch_state() {
    ...
    let writer_calls = std::cell::Cell::new(0);
    ...
    let changed = crate::app::save_document_with_spreadsheet_drafts(
        ...
        |staged| {
            ...
            assert_eq!(staged.get_spreadsheet_cell(0, 0), "");
    ...
    .expect("the final valid batch should save");
...
#[test]
fn spreadsheet_draft_save_reconciles_coordinate_objects_from_final_batch_sources() {
    let mut document = crate::app::initial_document();
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 0, "(1, 2)".to_string())
        .expect("seed A1 coordinate");
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "(3, 4)".to_string())
        .expect("seed B1 coordinate");
    let a1 = document
        .spreadsheet_coordinate_point("A1")
        .expect("A1 owns a point");
    let b1 = document
        .spreadsheet_coordinate_point("B1")
        .expect("B1 owns a point");
    let mut drafts =
        std::collections::HashMap::from([((0, 0), "(5, 6)".to_string()), ((0, 1), String::new())]);
    let mut undo_stack = Vec::new();
    let mut redo_stack = Vec::new();

    crate::app::save_document_with_spreadsheet_drafts(
        &mut document,
        &mut drafts,
        &mut undo_stack,
        &mut redo_stack,
        |staged| {
            assert!(matches!(
                staged.get_object(a1),
                Some(grafito_core::GeoObject::Point(point))
                    if point.position == grafito_geometry::Point2::new(5.0, 6.0)
            ));
            assert!(staged.get_object(b1).is_none());
            Ok(())
        },
    )
    .expect("coordinate batch should save");

    assert_eq!(document.spreadsheet_coordinate_point("A1"), Some(a1));
    assert_eq!(document.spreadsheet_coordinate_point("B1"), None);
    assert!(document.get_object(b1).is_none());
    assert!(drafts.is_empty());
    assert_eq!(undo_stack.len(), 1);
    assert!(redo_stack.is_empty());
}
...
fn spreadsheet_draft_save_clears_unresolved_final_batch_variables() {
    ...
    crate::app::save_document_with_spreadsheet_drafts(
        ...
        |staged| {
            assert_eq!(staged.get_variable("A1"), None);
            assert_eq!(staged.get_variable("B1"), None);
            assert_eq!(staged.variable_meta("A1"), None);
            assert_eq!(staged.variable_meta("B1"), None);
            Ok(())
        },
    )
    .expect("final cleared and invalid sources should save");

    assert_eq!(document.get_variable("A1"), None);
    assert_eq!(document.get_variable("B1"), None);
    assert_eq!(document.variable_meta("A1"), None);
    assert_eq!(document.variable_meta("B1"), None);
    assert_eq!(undo_stack.len(), 1);
    assert!(redo_stack.is_empty());
}

#[test]
fn spreadsheet_draft_save_batches_large_edits_into_one_document_revision() {
    ...
    for index in 0..CELL_COUNT {
        drafts.insert(
            (
                index / grafito_core::Document::MAX_SPREADSHEET_COLS,
                index % grafito_core::Document::MAX_SPREADSHEET_COLS,
            ),
            index.to_string(),
        );
    ...
    let changed = crate::app::save_document_with_spreadsheet_drafts(
        ...
        |staged| {
            writer_calls.set(writer_calls.get() + 1);
            assert_eq!(staged.get_variable("A1"), Some(0.0));
            assert_eq!(staged.get_variable("DH2"), Some((CELL_COUNT - 1) as f64));
            Ok(())
        },
    )
    .expect("bounded draft batch should save");
...
#[test]
fn spreadsheet_draft_write_failure_preserves_live_document_drafts_and_history() {
    let mut document = crate::app::initial_document();
    let before = serde_json::to_value(&document).expect("document should serialize");
    let mut drafts = std::collections::HashMap::from([((0, 0), "9".to_string())]);
    let drafts_before = drafts.clone();
    let mut undo_stack = Vec::new();
    let mut redo_stack = vec![grafito_core::ChangeSet {
        before: document.clone(),
        after: document.clone(),
    ...
    let result = crate::app::save_document_with_spreadsheet_drafts(
        &mut document,
        &mut drafts,
        &mut undo_stack,
        &mut redo_stack,
    ...
    assert_eq!(drafts, drafts_before);
    assert!(undo_stack.is_empty());
    assert_eq!(redo_stack.len(), 1);
}

#[test]
fn invalid_spreadsheet_draft_keeps_pending_action_and_never_calls_writer() {
    use crate::app::{DocumentAction, UnsavedDecision, UnsavedResolution};

    let mut document = crate::app::initial_document();
    crate::panels::apply_spreadsheet_cell_edit(&mut document, 0, 1, "(1, 2)".to_string())
        .expect("create coordinate cell");
    let before = serde_json::to_value(&document).expect("document should serialize");
    let mut drafts =
        std::collections::HashMap::from([((0, 0), "7".to_string()), ((0, 1), "(3,".to_string())]);
    let drafts_before = drafts.clone();
    let mut undo_stack = Vec::new();
    let mut redo_stack = vec![grafito_core::ChangeSet {
        before: document.clone(),
    ...
    let mut lifecycle = crate::app::DocumentLifecycle::new(&document);
    lifecycle.request_action(DocumentAction::Exit, &document, true);
    assert_eq!(
        lifecycle.resolve_unsaved_decision(UnsavedDecision::Save),
        Some(UnsavedResolution::Save(DocumentAction::Exit))
    );

    let error = crate::app::save_document_with_spreadsheet_drafts(
        &mut document,
        &mut drafts,
        &mut undo_stack,
        &mut redo_stack,
    ...
    .expect_err("incomplete coordinate draft must reject save");
    lifecycle.record_save_failure(error.clone());

    assert!(!writer_called.get());
    assert!(error.contains("B1"));
    assert_eq!(lifecycle.pending_action(), Some(DocumentAction::Exit));
    assert_eq!(lifecycle.save_error(), Some(error.as_str()));
    assert_eq!(
        serde_json::to_value(&document).expect("document should serialize"),
        before
    );
...
#[test]
fn invalid_staged_spreadsheet_save_preserves_live_state_and_skips_the_writer() {
    let mut valid_document = crate::app::initial_document();
    valid_document
        .try_set_variable("manual".to_string(), 1.0)
        .expect("fixture variable inserts");
    valid_document
        .configure_variable_animation("manual", 0.0, 2.0, 1.0, grafito_core::AnimationMode::Loop)
        .expect("fixture metadata is valid");
    let mut raw = serde_json::to_value(&valid_document).expect("fixture document serializes");
    raw["variable_meta"]["manual"]["min"] = serde_json::Value::from(2.0);
    let mut document: grafito_core::Document =
        serde_json::from_value(raw).expect("unchecked fixture document deserializes");
    let before = serde_json::to_value(&document).expect("document should serialize");
    let mut drafts = std::collections::HashMap::from([((0, 0), "7".to_string())]);
    let drafts_before = drafts.clone();
    let mut undo_stack = Vec::new();
    let mut redo_stack = vec![grafito_core::ChangeSet {
        before: document.clone(),
    ...
    .expect_err("invalid staged spreadsheet state must be rejected before writing");

    assert!(error.contains("VariableMeta manual.min"), "{error}");
    assert!(!writer_called.get());
    assert_eq!(
        serde_json::to_value(&document).expect("document should serialize"),
        before
...
#[test]
fn cancel_preserves_spreadsheet_drafts_and_replacement_is_the_discard_boundary() {
    use crate::app::{DocumentAction, UnsavedDecision, UnsavedResolution};

    let document = crate::app::initial_document();
    let drafts = std::collections::HashMap::from([((0, 0), "11".to_string())]);
    let drafts_before = drafts.clone();
    let mut lifecycle = crate::app::DocumentLifecycle::new(&document);
    lifecycle.request_action(DocumentAction::New, &document, true);
    ...
    let commit = replacement
        .find("self.document = document;")
        .expect("replacement commit");
    let clear = replacement
        .find("self.spreadsheet_edit_buffers.clear();")
        .expect("draft clear after replacement");
    assert!(commit < clear);
}

#[test]
fn deferred_file_actions_observe_spreadsheet_input_processed_later_in_the_frame() {
    use crate::app::{
        DeferredFileActions, DeferredFileIntent, DocumentAction, DocumentActionRequest,
        DocumentLifecycle, FileCommand,
    };
    ...
    for event in [
        FrameEvent::Command(FileCommand::Save),
        FrameEvent::Command(FileCommand::SaveAs),
        FrameEvent::Command(FileCommand::New),
        FrameEvent::Command(FileCommand::Open),
        FrameEvent::Command(FileCommand::Exit),
        FrameEvent::NativeClose,
    ] {
        let mut document = crate::app::initial_document();
        let mut lifecycle = DocumentLifecycle::new(&document);
        ...
        match event {
            FrameEvent::Command(command) => assert!(actions.queue_command(command)),
            FrameEvent::NativeClose => {
                assert!(actions.intercept_native_close(false));
                assert_eq!(
                    actions.pending(),
        ...
        // The spreadsheet consumes text/IME after the menu, shortcut, or OS event.
        drafts.insert((0, 0), "17".to_string());
        let intent = actions
            .take_after_editors()
            .expect("queued intent must survive until editors finish");
        let has_drafts = crate::app::spreadsheet_drafts_are_dirty(&document, &drafts);

        match intent {
            DeferredFileIntent::Command(command @ (FileCommand::Save | FileCommand::SaveAs)) => {
                crate::app::save_document_with_spreadsheet_drafts(
                    &mut document,
                    &mut drafts,
                    &mut undo_stack,
                    ...
                    |staged| {
                        writer_called.set(true);
                        assert_eq!(staged.get_spreadsheet_cell(0, 0), "17");
                        Ok(())
                    },
                )
                .expect("same-frame draft should be included in Save");
                lifecycle.record_save_success(
                    std::path::PathBuf::from(match command {
                        FileCommand::SaveAs => "/tmp/deferred-save-as.json",
                        FileCommand::Save => "/tmp/deferred-save.json",
                        _ => unreachable!(),
                    }),
                    &document,
                );
                assert!(writer_called.get());
                assert_eq!(document.get_spreadsheet_cell(0, 0), "17");
                assert!(drafts.is_empty());
                assert!(!lifecycle.is_dirty(&document, false));
            }
            DeferredFileIntent::Command(command) => {
                let action = match command {
                    FileCommand::New => DocumentAction::New,
                    FileCommand::Open => DocumentAction::Open,
                    FileCommand::Exit => DocumentAction::Exit,
                    FileCommand::Save | FileCommand::SaveAs => unreachable!(),
                };
                assert_eq!(
                    lifecycle.request_action(action, &document, has_drafts),
                    DocumentActionRequest::AwaitDecision(action)
                ...
                assert!(!writer_called.get());
                assert_eq!(document.get_spreadsheet_cell(0, 0), "");
                assert_eq!(drafts.get(&(0, 0)).map(String::as_str), Some("17"));
            }
            DeferredFileIntent::Decision(_) => panic!("no dialog decision was queued"),
        }

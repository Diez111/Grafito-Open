...
//!
//! - **Proxy**: las 21 tools puras de `grafito-assistant` (`all_safe_tool_schemas`
...
/// Definiciones MCP de las tools proxedas (21: 3 base + 8 pedag + 8 math + 2 harness1).
...
mod tests {
    ...
    #[test]
    fn proxy_cubre_21_tools_y_excluye_2_mas_web() {
        let defs = proxied_tool_defs();
        let names: Vec<&str> = defs
            .iter()
            .filter_map(|d| d.get("name").and_then(Value::as_str))
            .collect();
        assert_eq!(defs.len(), 21, "nombres: {names:?}");

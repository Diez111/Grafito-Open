...
| Lab MCP | `grafito-mcp` tools/resources | 33 tools (7 lab + execute + 21 proxedas assistant vía `SafeGrafitoDispatcher`; 2 Lean sidecar; 2 policy dream-loop; 1 gpu_probe WGSL verificado RADV+llvmpipe; `web_search` y harness-2 viejo excluidos) + 8 resources (ledger/run/cnf/result/bounds/catalog/lean-proof/policy-archive) por stdio | `crates/grafito-mcp/src/{lib,tools,protocol,ledger,sat,bridge,lean,policy,gpu}.rs` (MCP 2024-11-05; ledger XDG `lab_ledger.jsonl`; `execute_command` multi-step ≤32 con catch_unwind; SHA-lock anti-spec-gaming; topes 100k pts/2M aristas/500k vars/50 MiB/100k seeds/SAT 24h/Lean 600s) |
